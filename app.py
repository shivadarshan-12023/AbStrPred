...
def esm2_320_embed(sequence):
    ...
    # Convert input_ids to tensor if it's a list
    input_ids = torch.tensor(inputs['input_ids'])  # Ensure input_ids is a tensor
    
    with torch.no_grad():
        results = esm_model(input_ids, return_contacts=False)
...
def run_blast_and_get_dataframe(temp_fasta, blast_output):
    # Ensure the blastp binary has execute permission
    if not os.access(BLAST_PATH, os.X_OK):  # Check if it's executable
        raise PermissionError(f"The file {BLAST_PATH} is not executable.")
